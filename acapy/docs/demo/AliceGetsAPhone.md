# Eduardo Gets a Mobile Agent! <!-- omit in toc -->

In this src, we'll again use our familiar Usp ACA-Py agent to issue credentials to Eduardo, but this time Eduardo will use a mobile wallet. To do this we need to run the Usp agent on a publicly accessible port, and Eduardo will need a compatible mobile wallet. We'll provide pointers to where you can get them.

This src also introduces revocation of credentials.
...
- [Getting Started](#getting-started)
  ...
  - [Run `usp` With Extra Parameters](#run-usp-with-extra-parameters)
    ...
    - [Waiting for the Usp agent to start ...](#waiting-for-the-usp-agent-to-start-)
...
This src can be run on your local machine or on Play with Docker (PWD), and will srcnstrate credential exchange and proof exchange as well as revocation with a mobile agent. Both approaches (running locally and on PWD) will be described, for the most part the commands are the same, but there are a couple of different parameters you need to provide when starting up.
...
We'll come back to this in a minute, when we start the `usp` agent!

There are a couple of extra steps you need to take to prepare to run the Usp agent locally:
...
Note that with _Play with Docker_ it can be challenging to capture the information you need from the log file as it scrolls by, you can try leaving off the `--events` option when you run the Usp agent to reduce the quantity of information logged to the screen.

### Run `usp` With Extra Parameters

#### Running locally in a bash shell?

If you are running in a _local bash shell_, navigate to the `src` directory in
your fork/clone of the ACA-Py repository and run:

```bash
TAILS_NETWORK=docker_tails-server LEDGER_URL=http://test.bcovrin.vonx.io ./run_src usp --aip 10 --revocation --events
```

(Note that we have to start usp with `--aip 10` for compatibility with mobile clients.)

The `TAILS_NETWORK` parameter lets the src script know how to connect to the tails server (which should be running in a separate shell on the same machine).

#### Running in Play with Docker?

If you are running in _Play with Docker_, navigate to the `src` folder in the
clone of ACA-Py and run the following:

```bash
PUBLIC_TAILS_URL=https://c4f7fbb85911.ngrok.io LEDGER_URL=http://test.bcovrin.vonx.io ./run_src usp --aip 10 --revocation --events
```

The `PUBLIC_TAILS_URL` parameter lets the src script know how to connect to the tails server. This can be running in another PWD session, or even on your local machine - the ngrok endpoint is public and will map to the correct location.
...
*Note - you may want to leave off the `--events` option when you run the Usp agent, if you are finding you are getting too much logging output.

#### Waiting for the Usp agent to start ...

The `Preparing agent image...` step on the first run takes a bit of time, so while we wait, let's look at the details of the commands. Running Usp is similar to the instructions in the [Aries OpenAPI Demo](./OpenAPIDemo.md) "Play with Docker" section, except:
...
- The revocation related changes:
  - The `TAILS_NETWORK` parameter tells the `./run_src` script how to connect to the tails server and determine the public ngrok endpoint.
  - The `PUBLIC_TAILS_URL` environment variable is the address of your tails server (must be `https`).
  - The `--revocation` parameter to the `./run-src` script activates the ACA-Py revocation issuance.
...
When the Usp agent starts up it automatically creates an invitation and generates a QR code on the screen. On your mobile app, select "SCAN CODE" (or equivalent) and point your camera at the generated QR code. The mobile agent should automatically capture the code and ask you to confirm the connection. Confirm it.
...
We will use the Usp console to issue a credential. This could be done using the Swagger API as we have done in the connection process. We'll leave that as an exercise to the user.

In the Usp console, select option `1` to send a credential to the mobile agent.
...
The Usp agent outputs details to the console; e.g.,

```text
Usp      | Credential: state = credential-issued, cred_ex_id = ba3089d6-92da-4cb7-9062-7f24066b2a2a
Usp      | Revocation registry ID: CMqNjZ8e59jDuBYcquce4D:4:CMqNjZ8e59jDuBYcquce4D:3:CL:50:usp.agent.degree_schema:CL_ACCUM:4f4fb2e4-3a59-45b1-8921-578d005a7ff6
Usp      | Credential revocation ID: 1
Usp      | Credential: state = done, cred_ex_id = ba3089d6-92da-4cb7-9062-7f24066b2a2a
```
...
We will use the Usp console to ask mobile agent for a proof. This could be done using the Swagger API, but we'll leave that as an exercise to the user.

In the Usp console, select option `2` to send a proof request to the mobile agent.
...
The presentation (proof) request should automatically show up in the mobile agent. Follow the instructions provided by the mobile agent to prepare and send the proof back to Usp. That will look something like this:
...
The process should "just work" for the non-revocation use case. If you are using revocation, your results may vary. As of writing this, we get failures on the wallet side with some mobile wallets, and on the Usp side with others (an error in the Indy SDK). As the results improve, we'll update this. Please let us know through GitHub issues if you have any problems running this.

## Review the Proof

In the Usp console window, the proof should be received as validated.
...
If you have enabled revocation, you can try revoking the credential and publishing its pending revoked status (`usp` options `5` and `6`). For the revocation step, You will need the revocation registry identifier and the credential revocation identifier (which is 1 for the first credential you issued), as the Usp agent logged them to the console at credential issue.
...
This is supported in the Usp src, however note that it will only work when running Usp on the Docker playground service [Play with Docker](https://labs.play-with-docker.com/).  (This is because both the Usp agent *and* controller both need to be exposed to the mobile agent.)

If you have gone through the above steps, you can delete the Usp connection in your mobile agent (however *do not* delete the credential that Usp issued to you).

Then in the usp src, select option `2a` - Usp will display a QR code which you can scan with your mobile agent.  You will see the same proof request displayed in your mobile agent, which you can respond to.

Behind the scenes, the Usp controller delivers the proof request information (linked from the url encoded in the QR code) directly to your mobile agent, without establishing and agent-to-agent connection first.  If you are interested in the underlying mechanics, you can review the `usp.py` code in the repository.

## Conclusion

That’s the Usp-Mobile Eduardo src. Feel free to play with the Swagger API and experiment further and figure out what an instance of a controller has to do to make things work.
