...
Here's the flow that srcnstrates where reuse helps. For simplicity, we'll use the terms "Issuer"
...
The use of connection reuse can be srcnstrated with the Eduardo / Usp srcs as
follows. We assume you have already somewhat familiar with your options for
running the [Eduardo Usp Demo] (e.g. locally or in a browser). Follow those
instruction up to the point where you are about to start the Usp and Eduardo agents.

[Eduardo Usp Demo]: ../src/README.md

1. On a command line, run Usp with these parameters: `./run_src usp
   --reuse-connections --public-did-connections --events`.
2. On a second command line, run Eduardo as normal, perhaps with the `events`
   option: `./run_src eduardo --reuse-connections --events`
3. Copy the invitation from the Usp terminal and paste it into the Eduardo
   terminal at the prompt.
4. Verify that the connection was established.
   1. If you want, go to the Eduardo OpenAPI screen (port `8031`, path
      `api/docs`), and then use the `GET Connections` to see that Eduardo has one
      connection to Usp.
5. In the Usp terminal, type `4` to get a prompt for a new connection. This
   will generate a new invitation with the same public DID.
6. In the Eduardo terminal, type `4` to get a prompt for a new connection, and
   paste the new invitation.
7. Note from the webhook events in the Usp terminal that the `reuse` message
   is received from Eduardo, and as a result, no new connection was created.
   1. Execute again the `GET Connections` endpoint on the Eduardo OpenAPI screen
      to confirm that there is still just one established connection.
8. Try running the src again **without** the `--reuse-connections` parameter and
...
While in the src Usp uses in the invitation the same DID they publish as an
issuer (and uses in creating the schema and Cred Def for the src), Usp could
...
For example, to run usp with connection reuse using a non-public DID:

``` bash
./run_src usp --reuse-connections --events
```

To run usp using a `did:peer` and reusable connections:

``` bash
./run_src usp --reuse-connections --emit-did-peer-2 --events
```

To run this src using a multi-use invitation (from Usp):

``` bash
./run_src usp --reuse-connections --emit-did-peer-2 --multi-use-invitations --events
```
