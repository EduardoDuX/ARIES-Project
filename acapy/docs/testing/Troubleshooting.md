...
- Do you even need a local ledger? If not, you can use a public sandbox ledger,
  ...
  when running the Eduardo-Usp src in the [src](../src/README.md) folder, you can run (for
  example), the Usp agent using the command:
...
We add an integration test that srcnstrates/tests this issue [here](https://github.com/openwallet-foundation/acapy/blob/main/src/features/taa-txn-author-acceptance.feature#L67).
...
- Start the tails server as usual:
  - `./manage start --logs`
- To run the scenario manually, start usp and let the agent know it needs to TAA-accept before doing any ledger writes:
  - `./run_src usp --revocation --taa-accept`, and then you can run through all the transactions using the Swagger page.
