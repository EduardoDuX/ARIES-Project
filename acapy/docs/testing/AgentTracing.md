...
## Enabling Tracing in the Eduardo/Usp Demo

The `run_src` script supports the following parameters and environment variables.
...
When running the Usp controller, tracing can be enabled using the `T` menu option:
...
You can use the `ELK` stack in the [ELK Stack sub-directory](https://github.com/openwallet-foundation/acapy/blob/main/src/elk-stack) as a target for trace events, just start the ELK stack using the docker-compose file and then in two separate bash shells, startup the src as follows:

```bash
DOCKER_NET=elknet TRACE_TARGET_URL=logstash:9700 ./run_src usp --trace-http
```

```bash
DOCKER_NET=elknet TRACE_TARGET_URL=logstash:9700 ./run_src eduardo --trace-http
```

## Hooking into event messaging

ACA-PY supports sending events to web hooks, which allows the src agents to display them in the CLI. To also send them to another end point, use the `--webhook-url` option, which requires the `WEBHOOK_URL` environment variable. Configure an end point running on the docker host system, port *8888*, use the following:

```bash
WEBHOOK_URL=host.docker.internal:8888 ./run_src usp --webhook-url
```
