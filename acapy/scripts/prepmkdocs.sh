...
if [[ "$1" == "clean" ]]; then
  rm -f docs/CHANGELOG.md \
    ...
    git checkout -- docs/README.md docs/src/OpenAPIDemo.md docs/src/EduardoGetsAPhone.md docs/features/DevReadMe.md
else
    ...
    # Fix image references in src documents so they work in GitHub and mkdocs
    for i in docs/src/OpenAPIDemo.md docs/src/EduardoGetsAPhone.md; do sed -e "s#src=.collateral#src=\"../collateral#" $i >$i.tmp; mv $i.tmp $i; done
