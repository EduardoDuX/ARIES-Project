...
The following changes were made to better incorporate this stack into ACA-Py srcs and tracing.
...
We can run srcs to see agent tracing events and attach them to the `elknet` network to push events to ELK.

## srcs

Assuming the elk stack is running from above... from your srcs directory, in two separate bash shells, startup the src as follows:

```bash
DOCKER_NET=elknet TRACE_TARGET_URL=logstash:9700 LEDGER_URL=http://test.bcovrin.vonx.io ./run_src usp --trace-http
```

```bash
DOCKER_NET=elknet TRACE_TARGET_URL=logstash:9700 LEDGER_URL=http://test.bcovrin.vonx.io ./run_src eduardo --trace-http
```

And run the src scenarios as you wish.

**Note**: the `LEDGER_URL` override is unnecessary for tracing; it is only used so `usp` and `eduardo` are on the same ledger as the following `multi-src`. 

## multi-src

The `multi-src` (running ACA-Py in multitenant mode) requires a few edits to [docker-compose.yml](../multi-src/docker-compose.yml). 
...
Assuming the elk stack is running from above... from your multi-src directory, startup the agent as follows:
...
![Animated src](https://user-images.githubusercontent.com/3299086/155972072-0c89d6db-707a-47a1-818b-5f976565f95a.gif)
...
- [ACA-Py ELK Stack for srcs](#aca-py-elk-stack-for-srcs)
  - [run](#run)
  - [srcs](#srcs)
  - [multi-src](#multi-src)
