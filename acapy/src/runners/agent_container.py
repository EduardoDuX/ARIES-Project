...
class AriesAgent(DemoAgent):
    ...
    async def generate_invitation(
        ...
        with log_timer("Generate invitation duration:"):
            ...
            log_status("#7 Create a connection to eduardo and print out the invite details")
        ...
        print("Envie o convite JSON abaixo a quem quiser conversar:")
        ...
        if display_qr:
            ...
            log_msg(
                "Use the following JSON to accept the invite from another src agent."
...
def arg_parser(ident: str = None, port: int = 8020):
    ...
    "ident", if specified, refers to one of the standard src personas - eduardo, usp, acme or performance.
    """
    parser = argparse.ArgumentParser(
        description="Runs a " + (ident or "aries") + " src agent."
    )
    if not ident:
        parser.add_argument(
            "--ident",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "here: ./src/multi_ledger_config.yml"
        ),
    )
    parser.add_argument(
        "--wallet-type",
    ...
    if (not ident) or (ident != "eduardo"):
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Use Usp public key in the invite. "
                "Only applicable for AIP 2.0 (OOB) connections."
            ),
        )
        parser.add_argument(
...
async def create_agent_with_args(args, ident: str = None, extra_args: list = None):
    ...
    if "multi_ledger" in args and args.multi_ledger:
        multi_ledger_config_path = "./src/multi_ledger_config.yml"
    else:
    ...
    log_msg(
        f"Initializing src agent {agent_ident} with AIP {aip} and credential type {cred_type}"
    )
...
async def test_main(
    ...
    usp_container = None
    eduardo_container = None
    try:
        # initialize the containers
        usp_container = AgentContainer(
            genesis_txns=genesis,
            ident="Usp.agent",
        ...
        eduardo_container = AgentContainer(
            genesis_txns=genesis,
            ident="Eduardo.agent",
        ...
        # start the agents - usp gets a public DID and schema/cred def
        await usp_container.initialize(
        ...
        await eduardo_container.initialize()

        # usp create invitation
        invite = await usp_container.generate_invitation()

        # eduardo accept invitation
        invite_details = invite["invitation"]
        await eduardo_container.input_invitation(invite_details)

        # wait for usp connection to activate
        await usp_container.detect_connection()
        await eduardo_container.detect_connection()

        # TODO usp issue credential to eduardo
        # TODO eduardo check for received credential
        ...
        try:
            ...
            if eduardo_container:
                log_msg("Shutting down eduardo agent ...")
                await eduardo_container.terminate()
            if usp_container:
                log_msg("Shutting down usp agent ...")
                await usp_container.terminate()
        except Exception as e:
