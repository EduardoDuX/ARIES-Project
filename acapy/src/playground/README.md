...
This directory contains scripts to run several ACA-Py agents in various configurations for srcnstration, development and testing scenarios. The agents are using Postgres (15) database storage (`askar`, `DatabasePerWallet`) and are running without security (`--admin-insecure-mode`).
...
- [ngrok-usp-eduardo](./ngrok-usp-eduardo.yml)
...
docker compose up usp-agent eduardo-agent
```

You can now access the agent Admin APIs via Swagger at:

- usp: [http://localhost:9011/api/doc#](http://localhost:9011/api/doc#)
- eduardo: [http://localhost:9012/api/doc#](http://localhost:9012/api/doc#)
