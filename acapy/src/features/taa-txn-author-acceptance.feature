...
# you can run von-network as `./manage start --taa-sample --logs`
   ...
   Scenario Outline: Revoke credential using a ledger with TAA required
      ...
      And "Usp" connects to a ledger that requires acceptance of the TAA
      And "Usp" accepts the TAA
      And "Usp" and "Bob" have an existing connection
      And "Bob" has an issued <Schema_name> credential <Credential_data> from "<issuer>"
      And "Usp" revokes the credential
      And "Usp" successfully revoked the credential
      ...
      Examples:
         ...
         | Usp  | --taa-accept --revocation --public-did   |                  | driverslicense_v2 | Data_DL_MaxValues | DL_age_over_19_v2 |
   ...
   Scenario Outline: Fail to publish revoked credential using a ledger with TAA required, and fix the ledger
      ...
      And "Usp" revokes the credential
      And "Usp" successfully revoked the credential
      When "Usp" rejects the TAA
      And "Bob" has an issued <Schema_name> credential <Credential_data> from "<issuer>"
      And "Usp" attempts to revoke the credential
      And "Usp" fails to publish the credential revocation
      Then "Usp" accepts the TAA
      And "Usp" posts a revocation correction to the ledger
      And "Usp" successfully revoked the credential
      And "Bob" has an issued <Schema_name> credential <Credential_data> from "<issuer>"
      And "Usp" revokes the credential
      And "Usp" successfully revoked the credential
   ...
   Scenario Outline: Fail to publish revoked credential using a ledger with TAA required, and fix the ledger manually before revoking more credentials
      ...
      And "Usp" fails to publish the credential revocation
      And "Usp" accepts the TAA
      Then "Usp" posts a revocation correction to the ledger
      And "Usp" successfully revoked the credential
      And "Bob" has an issued <Schema_name> credential <Credential_data> from "<issuer>"
      And "Usp" revokes the credential
      And "Usp" successfully revoked the credential
