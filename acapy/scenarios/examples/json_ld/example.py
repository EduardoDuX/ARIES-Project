    ...
    async with Controller(base_url=ALICE) as eduardo, Controller(base_url=BOB) as bob:
        ...
        with section("Prepare for issuance"):
            with section("Issuer prepares issuing DIDs", character="-"):
                ...
                public_did = (
                    await eduardo.get("/wallet/did/public", response=DIDResult)
                ).result
                if not public_did:
                    public_did = (
                        await eduardo.post(
                ...
                assert bls_eduardo_did_res
                bls_eduardo_did = bls_eduardo_did_res["did"]
        ...
        with section("Present example credential"):
            eduardo_pres_ex, bob_pres_ex = await jsonld_present_proof(
                eduardo,
                bob,
                eduardo_conn.connection_id,
                bob_conn.connection_id,
        ...
        with section("Presentation summary", character="-"):
            print(presentation_summary(eduardo_pres_ex.into(V20PresExRecord)))
