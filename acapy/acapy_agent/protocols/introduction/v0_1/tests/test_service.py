...
from .. import base_service, src_service
...
class TestIntroductionService(IsolatedAsyncioTestCase):
    ...
    async def test_service_start_introduction_no_init_conn_rec(self):
        service = await src_service.DemoIntroductionService.service_handler()()
    ...
    async def test_service_start_and_return_introduction(self):
        ...
        assert isinstance(result, src_service.IntroInvitationRequest)
        assert result.message == "Hello Start"
        assert target["connection_id"] == conn_rec_target._id

        invite = src_service.IntroInvitation(
        ...
        assert len(messages) == 1
        (result, target) = messages[0]
        assert isinstance(result, src_service.ForwardInvitation)
